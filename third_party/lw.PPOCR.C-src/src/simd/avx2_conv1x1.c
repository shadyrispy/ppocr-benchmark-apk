...
#  include <immintrin.h>
#  define LW_COMPILES_AVX2_CONV1X1 1
#else
#  define LW_COMPILES_AVX2_CONV1X1 0
#endif

#if LW_COMPILES_AVX2_CONV1X1 && (defined(__GNUC__) || defined(__clang__))
            ...
            for (output_channel_in_group = 0u; output_channel_in_group < output_channels_per_group;
                ...
                for (input_channel_in_group = 0u; input_channel_in_group < input_channels_per_group;
                    ...
                    __m256 weight_values =
                        _mm256_set1_ps(output_channel_weights[input_channel_in_group]);
                    spatial = 0u;
                    for (; spatial + 8u <= channel_plane; spatial += 8u) {
                        __m256 output_values =
                            _mm256_loadu_ps(output_channel_data + (size_t)spatial);
                        __m256 input_values = _mm256_loadu_ps(input_channel_data + (size_t)spatial);
                        output_values = _mm256_add_ps(output_values,
                                                      _mm256_mul_ps(input_values, weight_values));
                        _mm256_storeu_ps(output_channel_data + (size_t)spatial, output_values);
                    }
                    for (; spatial < channel_plane; ++spatial) {
