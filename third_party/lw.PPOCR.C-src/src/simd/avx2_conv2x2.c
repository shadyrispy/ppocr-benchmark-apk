...
static void conv2x2_unit_pad_end1_four_outputs_f32(
    ...
    for (batch = 0u; batch < (uint32_t)input_dimensions[0]; ++batch) {
        ...
        for (output_channel = 0u; output_channel < (uint32_t)output_dimensions[1];
            ...
            __m256 initial_values0 = _mm256_set1_ps(initial0);
            __m256 initial_values1 = _mm256_set1_ps(initial1);
            __m256 initial_values2 = _mm256_set1_ps(initial2);
            __m256 initial_values3 = _mm256_set1_ps(initial3);
            uint64_t spatial = 0u;
            for (; spatial + 8u <= channel_plane; spatial += 8u) {
                _mm256_storeu_ps(output0 + (size_t)spatial, initial_values0);
                _mm256_storeu_ps(output1 + (size_t)spatial, initial_values1);
                _mm256_storeu_ps(output2 + (size_t)spatial, initial_values2);
                _mm256_storeu_ps(output3 + (size_t)spatial, initial_values3);
            }
                ...
                for (kernel_y = 0u; kernel_y < 2u; ++kernel_y) {
                    ...
                    for (output_y = 0u; output_y < output_y_end; ++output_y) {
                        ...
                        for (kernel_x = 0u; kernel_x < 2u; ++kernel_x) {
                            ...
                            __m256 weight0 = _mm256_set1_ps(weights0_channel[kernel_y * 2u + kernel_x]);
                            __m256 weight1 = _mm256_set1_ps(weights1_channel[kernel_y * 2u + kernel_x]);
                            __m256 weight2 = _mm256_set1_ps(weights2_channel[kernel_y * 2u + kernel_x]);
                            __m256 weight3 = _mm256_set1_ps(weights3_channel[kernel_y * 2u + kernel_x]);
                            const float* input_values = input_row + kernel_x;
                            for (; output_x + 8u <= output_x_end; output_x += 8u) {
                                __m256 values = _mm256_loadu_ps(input_values + output_x);
                                __m256 values0 = _mm256_loadu_ps(output_row0 + output_x);
                                __m256 values1 = _mm256_loadu_ps(output_row1 + output_x);
                                __m256 values2 = _mm256_loadu_ps(output_row2 + output_x);
                                __m256 values3 = _mm256_loadu_ps(output_row3 + output_x);
                                _mm256_storeu_ps(output_row0 + output_x,
                                                 _mm256_add_ps(values0, _mm256_mul_ps(values, weight0)));
                                _mm256_storeu_ps(output_row1 + output_x,
                                                 _mm256_add_ps(values1, _mm256_mul_ps(values, weight1)));
                                _mm256_storeu_ps(output_row2 + output_x,
                                                 _mm256_add_ps(values2, _mm256_mul_ps(values, weight2)));
                                _mm256_storeu_ps(output_row3 + output_x,
                                                 _mm256_add_ps(values3, _mm256_mul_ps(values, weight3)));
                            }
